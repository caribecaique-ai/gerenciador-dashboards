...
const readDashboardSlug = (): string => {
  if (typeof window === "undefined") return "";
  return String(new URLSearchParams(window.location.search).get("slug") || "").trim();
};

const hasDashboardAccess = (rawToken?: string | null): boolean =>
  Boolean(normalizeToken(rawToken) || readDashboardSlug());
...
export const useClickUpData = (refreshMs = 30000, dashboardFilters?: Partial<DashboardFilters>) => {
  ...
  const loadNavigation = useCallback(
    async (teamId: string, token?: string | null, force = false): Promise<NavigationNode[]> => {
      if (!teamId || !hasDashboardAccess(token)) return [];
      ...
      const response = await getNavigation(teamId, token || undefined, force);
  ...
  const loadDashboardForScope = useCallback(
    async (
      ...
      if (!hasDashboardAccess(token)) return undefined;
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        const urlSlug = String(urlParams.get("slug") || "").trim();
        ...
        if (!tokenToUse && !urlSlug) {
          setError("Acesso ausente. Abra o dashboard pela URL gerada no manager.");
          return;
        }
        ...
        if (!active) return;
        if (tokenToUse && tokenToUse !== authTokenRef.current) {
  ...
  useEffect(() => {
    ...
    if (!selectedTeamId || !hasDashboardAccess(authTokenRef.current)) return;
    setError(null);
  ...
  useEffect(() => {
    if (!selectedTeamId || !hasDashboardAccess(authToken)) return undefined;

    const intervalId = setInterval(() => {
      const token = normalizeToken(authTokenRef.current);
      void loadNavigation(selectedTeamId, token || undefined, true);
      void loadDashboardForScope(selectedTeamId, selectedScope, { silent: true, force: true }, token || undefined);
    }, refreshMs);
  ...
  const changeTeam = useCallback(
    async (teamId: string) => {
      ...
      setError(null);
      let tree = await loadNavigation(teamId, token || undefined, false).catch(() => []);
      const payload = await loadDashboardForScope(teamId, teamScope, { silent: false, force: false }, token || undefined);
      if (!tree.length) {
        tree = await loadNavigation(teamId, token || undefined, true).catch(() => []);
      }
      if (!tree.length) {
  ...
  const prefetchScope = useCallback(
    async (scope: ScopeSelection) => {
      if (!selectedTeamId) return;
      const token = normalizeToken(authTokenRef.current);
      if (!hasDashboardAccess(token)) return;
  ...
  const refreshNow = useCallback(async () => {
    ...
    const tree = await loadNavigation(selectedTeamId, token || undefined, true).catch(() => []);
