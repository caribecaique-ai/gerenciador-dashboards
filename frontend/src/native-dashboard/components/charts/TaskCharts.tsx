...
interface MultiTrendWavePoint {
  label: string;
  value: number | null;
}

interface MultiTrendWaveSeries {
  id: string;
  label: string;
  color: string;
  points: MultiTrendWavePoint[];
}

interface MultiTrendWaveChartProps {
  series: MultiTrendWaveSeries[];
  smooth?: boolean;
}
...
export const MultiTrendWaveChart = ({
  series,
  smooth = true,
}: MultiTrendWaveChartProps) => {
  const chartType = smooth ? "monotone" : "linear";
  ...
  const normalizedSeries = useMemo(
    () =>
      (series || [])
        .map((entry) => ({
          id: String(entry.id || entry.label || "serie"),
          label: String(entry.label || entry.id || "Serie"),
          color: entry.color || "#00f3ff",
  ...
  const normalizedData = useMemo(() => {
    ...
    normalizedSeries.forEach((entry) => {
      entry.points.forEach((point) => labels.add(point.label));
    });

    return Array.from(labels).map((label) => {
      const row: Record<string, string | number | null> = { label };
      normalizedSeries.forEach((entry) => {
        const matchedPoint = entry.points.find((point) => point.label === label);
        row[entry.id] = matchedPoint?.value ?? null;
      });
      return row;
    });
  }, [normalizedSeries]);
  ...
  const seriesLabelMap = useMemo(
    () => new Map(normalizedSeries.map((entry) => [entry.id, entry.label])),
    [normalizedSeries]
  );

  if (!normalizedSeries.length || !normalizedData.length) {
    return (
      <div className="flex h-full min-h-[180px] items-center justify-center rounded border border-dashed border-white/10 font-mono text-[10px] uppercase tracking-[0.1em] text-slate-500">
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={normalizedData} margin={{ top: 12, right: 18, left: 8, bottom: 8 }}>
        <CartesianGrid strokeDasharray="3 5" stroke="rgba(0,243,255,0.08)" vertical={false} />
        <XAxis
          dataKey="label"
          tick={{ fill: "#8aa1bb", fontSize: 9, fontFamily: "JetBrains Mono" }}
          axisLine={{ stroke: "rgba(255,255,255,0.08)" }}
        ...
        <Tooltip
          ...
          formatter={(value: unknown, key: string | undefined) => {
            const numeric = toFiniteNumber(value);
            const safeKey = String(key || "");
            return [numeric === null ? "-" : formatCompactNumber(numeric, 1), seriesLabelMap.get(safeKey) || safeKey];
          }}
        />
        ...
        {normalizedSeries.map((entry) => (
          <Line
            ...
            strokeOpacity={0.86}
            dot={false}
            activeDot={{ r: 2.6, stroke: "#ffffff", strokeWidth: 1 }}
            connectNulls
            isAnimationActive={false}
