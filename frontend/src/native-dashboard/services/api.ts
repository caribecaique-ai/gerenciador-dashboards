...
const readDashboardSlug = () => {
  if (typeof window === "undefined") return "";
  return String(new URLSearchParams(window.location.search).get("slug") || "").trim();
};

const resolveHostAwareApiUrl = (envValue: string | undefined, fallbackPort: string) => {
  if (typeof window === "undefined") return envValue || `http://localhost:${fallbackPort}/api`;

  const browserHost = window.location.hostname;
  const browserIsLocal = isLocalHostname(browserHost);
  const fallbackUrl = `http://${browserHost}:${fallbackPort}/api`;
...
const resolveApiBaseUrl = () => {
  const dashboardSlug = readDashboardSlug();
  if (dashboardSlug) {
    return resolveHostAwareApiUrl(import.meta.env.VITE_API_URL, "3005");
  }
...
export const updateDashboardTaskStatus = async ({
  taskId,
  status,
  token,
}: UpdateTaskStatusParams) => {
  const response = await apiClient.put(
    `/dashboard/tasks/${encodeURIComponent(taskId)}/status`,
    { status },
    {
      params: {
        ...buildTokenParam(token),
